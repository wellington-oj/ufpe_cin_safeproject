...
public class CodeGeneration extends java.lang.Object {
    ...
    public void createExceptionTable(SafeStmt safeStmt) {  
        for(int i = 0; i < safeStmt.getNumCatchClause(); i++) {
          safeStmt.getCatchClause(i).exceptionTableEntries(this, safeStmt);
        }
        if(safeStmt.hasFinally()) {
          addException(
              addressOf(safeStmt.label_begin()),
              addressOf(safeStmt.label_finally()),
              addressOf(safeStmt.label_exception_handler()),
              0
              );
        }
      }
